...
class GaussiansManeger():
    def __init__(self, xyz, features, opacity, scales, rotations, lrs, not_legacy = True):
        if not_legacy:
            # s_min, s_max = 0.001, 1

            # sigmoid(x) + (1 -  sigmoid(x)) * 0.001
            ...
            self._scales = nn.Parameter(inverse_sigmoid(scales).squeeze(0).contiguous().float().detach().clone().requires_grad_(True))
    ...
    def __call__(self):
        if self.not_legacy:
            ...
            scales = torch.sigmoid(self._scales)
    ...
    def densify_and_split(self, grads, grad_threshold=0.04, scene_extent=2, N=2):
        ...
        if self.not_legacy:
            selected_pts_mask = torch.logical_and(selected_pts_mask,
                torch.max(torch.sigmoid(self._scales), dim=1).values > self.percent_dense * scene_extent
            )
            stds = torch.sigmoid(self._scales[selected_pts_mask]).repeat(N, 1)
        ...
        if self.not_legacy:
            new_scales = (torch.sigmoid(self._scales[selected_pts_mask]) / (0.8*N)).repeat(N, 1)
        else:
