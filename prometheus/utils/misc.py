...
__all__ = [
    ...
    'parse_file_format', 'set_cache_dir', 'get_cache_dir', 
    'download_url','gather_data'
]

# REPO_NAME = 'Code'  # Name of the repository (project).
REPO_NAME = 'PlatonicGen'  # Name of the repository (project).
...
def linear_schedule(step: int, val_start: float, val_end: float, period: int):
    """Get the current value from `val_from` to `val_to` if the linear growth
    over `period` steps is used. If out of range, then returns the boundary
    value."""
    if step >= period:
        return val_end
    elif step <= 0:
