"""
Trajectory Generation System for camera trjajectory generation
"""
...
from prometheus.modules.dit import SelfAttention, CrossAttention, Mlp, TimestepEmbedder, modulate
from prometheus.modules.renderers.gaussians_renderer import quaternion_to_matrix, matrix_to_quaternion


__all__ = ["TrajDiTSystem", "DiTBlock", "TrajDiTModel"]

class TrajDiTSystem(LightningModule):
    """TrajDiTSystem"""
    ...
    def inference(self, text, num_inference_steps=100, image_size=512, return_each=False):
        if isinstance(text, str):
            ...
            raise ValueError('Unsupport input type of text')
        ...
        for i, t in tqdm.tqdm(enumerate(timesteps), total=len(timesteps), desc='Denoising camera trajectory...'):
            ...
            tokens_noisy = self.scheduler.step(tokens_pred.cpu(), t[0].cpu(), tokens_noisy.cpu(), eta=0).prev_sample.to(self.device)
...
class TrajDiTModel(nn.Module):
    def __init__(self, opt):
        ...
        self.opt = opt
        if hasattr(opt, 'tokenizer'):
            self.tokenizer = opt.tokenizer
            self.text_encoder = opt.text_encoder.requires_grad_(False)
        else:
