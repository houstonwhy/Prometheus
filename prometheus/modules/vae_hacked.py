...
class CheckNaN:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        if self.first_nan_location is not None:
            print(f"最早出现 NaN 的位置: {self.first_nan_location}")

    def check(self, tensor, name):
        if torch.isnan(tensor).any():
            if self.first_nan_location is None:
                self.first_nan_location = name
            print(f"警告: {name} 包含 NaN")

# 定义装饰器来检查是否包含 NaN
def check_nan(module_name):
    def decorator(func):
        def wrapper(*args, **kwargs):
            with CheckNaN() as check_nan:
                result = func(*args, **kwargs)
                check_nan.check(result, f"{module_name}.{func.__name__}")
            return result
        return wrapper
    return decorator
...
class AutoencoderKL(torch.nn.Module):
    ...
    def decode(self, z, extra_z=None, path='new', post_conv = True):
