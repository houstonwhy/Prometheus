...
class CheckpointFunction(torch.autograd.Function):
    ...
    def backward(ctx, *output_grads):
        ...
        with torch.enable_grad(), torch.amp.autocast('cuda', **ctx.gpu_autocast_kwargs):
...
class SpatialTransformer3D(nn.Module):
    ''' 3D self-attention ''' 
    def __init__(self, in_channels, n_heads, d_head,
                 depth=1, dropout=0., disable_3d_attn=False, context_dim=None,
                 disable_self_attn=False, use_linear=False,
                 use_checkpoint=True):
        super().__init__()
        if exists(context_dim) and not isinstance(context_dim, list):
        ...
        if not use_linear:
            ...
            self.proj_out = zero_module(Linear(in_channels, inner_dim))
        self.use_linear = use_linear
        self.disable_3d_attn = disable_3d_attn
    ...
    def forward(self, x, context=None, num_frames=1):
        ...
        if self.use_linear:
            x = self.proj_in(x)
        for i, block in enumerate(self.transformer_blocks):
            x = block(x, context=context[i], num_frames=1 if self.disable_3d_attn else num_frames)
        if self.use_linear:
            x = self.proj_out(x)
        x = rearrange(x, 'b (h w) c -> b c h w', h=h, w=w).contiguous()
        if not self.use_linear:
...
class MultiViewUNetModel(nn.Module):
    ...
    def __init__(
        ...
        if num_head_channels == -1:
            assert num_heads != -1, 'Either num_heads or num_head_channels has to be set'
        #TODO hard codeed
        num_attn_blocks = 3 * num_res_blocks + 1 + 3 * (num_res_blocks+ 1) 
        if not disable_3d_attentions:
            self.disable_3d_attentions = [0] * num_attn_blocks
        else:
            assert len(disable_3d_attentions) == num_attn_blocks
            self.disable_3d_attentions = disable_3d_attentions
        self.image_size = image_size
        self.in_channels = in_channels
        self.model_channels = model_channels
        self.out_channels = out_channels
                    ...
                    if not exists(num_attention_blocks) or nr < num_attention_blocks[level]:
                        layers.append(
                            AttentionBlock(
                                ...
                                disable_self_attn=disabled_sa, use_linear=use_linear_in_transformer,
                                use_checkpoint=use_checkpoint,
                                disable_3d_attn=self.disable_3d_attentions.pop(0)
                            )
                        )
                self.input_blocks.append(TimestepEmbedSequential(*layers))
                self._feature_size += ch
                input_block_chans.append(ch)
            if level != len(channel_mult) - 1: # if not last level add conv
                out_ch = ch
                self.input_blocks.append(
                    TimestepEmbedSequential(
                        ResBlock(
