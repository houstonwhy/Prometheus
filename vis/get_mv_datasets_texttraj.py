...
def run(cfg: DictConfig) -> None:
    ...
    dataset = import_str(cfg['dataset']['mv_dataset']['module'])(
        **cfg['dataset']['mv_dataset']['args'],
        fake_length=10000 * 4,
        debug=False
    )
    ...
    # dataset[0].datasets
    os.makedirs(os.path.join(cfg.output_dir, 'scene_traj'), exist_ok = True)
    for batch in tqdm(data_loader):
        for i in range(batch_size):
            scene_dd = {
                'dataset_name':batch['dataset_name_mv'][i],
                'scene_name':batch['scene_name_mv'][i],
                'text':batch['text_mv'][i],
                'cameras':batch['cameras_mv'][i].numpy(),
                'images':batch['images_mv'][i].numpy()
            }
            spath = os.path.makedir(os.path.join(cfg.output_dir, 'scene_traj', batch['dataset_name_mv'][i] + "_" + batch['scene_name_mv'][i] + 'pkl'))
            with open(spath, 'wb') as fp:
                pkl.dump(fp, scene_dd)
